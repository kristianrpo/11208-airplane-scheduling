...
## entradas/salidas (antes de ingresar a la funcion), luego, si la posicion adyasente a el actual es casilla en blanco, se añade esta en el inicio del deque, y se asigna el mismo peso del actual,
## si es parqueadero, se añade en el final del deque, y se asigna el peso del actual + 1, además se guardan las coordenadas de los mismos para recorrerlos en un futuro. Y, si es bloqueo, no se añade al deque y se asigna el valor de infinito (-11).
...
## si el evento es negativo, busca una salida disponible. De no ser posible la salida o el ingreso de un avión, se realiza backtracking, acomodando nuevamente los
## los mismos hasta encontrar una solución factible.
...
def principal():
    ...
    while(True):
        ...
        if numero_aviones == 0:
            ...
            matriz_ponderar,parqueaderos = pre_ponderacion(matriz_simbolos,numero_filas,numero_columnas)
            resultado = resolver_problema(matriz_simbolos, matriz_ponderar, eventos, numero_filas, numero_columnas, lista_solucion, diccionario,parqueaderos)
