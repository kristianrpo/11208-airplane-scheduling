...
## si es parqueadero, lo añado en el final de mi deque, y le asigno el peso de mi actual + 1, y si es bloqueo, no lo añado al deque y le asigno el valor de infinito (-11). finalmente se retorna una lista con los parqueaderos por orden de peso.
def ponderar (matriz_simbolos,matriz_ponderar,pesos,movimientos,filas,columnas):
    ...
    parqueaderos_ordenados = sorted(parqueaderos,key = lambda x: x[1],reverse = True)
...
def resolver_problema(matriz_simbolos, matriz_ponderar, eventos, filas, columnas, lista_solucion, diccionario,parqueaderos_ordenados):
    if len(eventos)==0:
        return True
    else:
        evento_actual = eventos.popleft()
        if evento_actual>0:
            for i in range(len(parqueaderos_ordenados)):
                    if casilla_es_parqueadero(matriz_simbolos,parqueaderos_ordenados[i][0][0],parqueaderos_ordenados[i][0][1]) and matriz_ponderar[parqueaderos_ordenados[i][0][0]][parqueaderos_ordenados[i][0][1]]>0:
                        temporal = matriz_simbolos[parqueaderos_ordenados[i][0][0]][parqueaderos_ordenados[i][0][1]]
                        diccionario[evento_actual] = ((parqueaderos_ordenados[i][0][0],parqueaderos_ordenados[i][0][1]),temporal)
                        matriz_simbolos[parqueaderos_ordenados[i][0][0]][parqueaderos_ordenados[i][0][1]] = '##'
                        lista_solucion.append(temporal)
                        matriz_ponderar,parqueaderos_ordenados = pre_ponderacion(matriz_simbolos,filas,columnas)
                        if resolver_problema(matriz_simbolos, matriz_ponderar, eventos, filas, columnas, lista_solucion, diccionario,parqueaderos_ordenados):
                            return True
                        else:
                            lista_solucion.pop()
...
def principal():
    ...
    while(True):
        ...
        if numero_aviones == 0:
            ...
            ponderacion_inicial,parqueaderos_ordenados = pre_ponderacion(matriz_simbolos,numero_filas,numero_columnas)
            resultado = resolver_problema(matriz_simbolos, ponderacion_inicial, eventos, numero_filas, numero_columnas, lista_solucion, diccionario,parqueaderos_ordenados)
            if resultado:
